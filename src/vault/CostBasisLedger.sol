...
abstract contract CostBasisLedger {
    ...
    function _updateCostBasisOnSharesTransferred(address _from, address _to, uint256 _fromTotalSharesBeforeTransfer, uint256 _totalSharesTransferred) internal {
        CostBasisLedgerStorage storage $ = _getCostBasisLedgerStorage();

        uint256 totalCostBasisOwnedByFrom = $.userToCostBasisOfSharesOwned[_from];
        // TODO: Justfiy rounding direction
        uint256 costBasisForSharesTransferred =
            Math.mulDiv(totalCostBasisOwnedByFrom, _totalSharesTransferred, _fromTotalSharesBeforeTransfer, Math.Rounding.Floor);
