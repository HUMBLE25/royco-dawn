...
import { AccountingState, AssetClaims, ExecutionModel, RequestRedeemSharesBehavior } from "../../libraries/Types.sol";
...
interface IRoycoKernel {
    ...
    function syncTrancheNAVs() external returns (AccountingState memory state, AssetClaims memory claims);

    function previewSyncTrancheNAVs(TrancheType _trancheType) external view returns (AccountingState memory state, AssetClaims memory claims);
