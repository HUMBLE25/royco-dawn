 ...
 * @dev The NAV is expressed in tranche units with RAY (27 decimals) precision
...
abstract contract InKindAssetsQuoter is RoycoKernel {
    /// @notice The scaling factor to convert ST tranche units to and from RAY precision
    uint256 internal immutable ST_SCALE_FACTOR_TO_RAY;

    /// @notice The scaling factor to convert JT tranche units to and from RAY precision
    uint256 internal immutable JT_SCALE_FACTOR_TO_RAY;

    /// @notice Thrown when the senior or junior tranche asset has over RAY decimals of precision
    ...
    constructor() {
        ...
        require(stDecimals <= RAY_DECIMALS && jtDecimals <= RAY_DECIMALS, UNSUPPORTED_DECIMALS());

        // Compute the scaling factor that will scale each tranche's asset quantities to and from RAY precision
        // The NAV unit of this quoter is the tranche asset (the same for both tranches)
        ST_SCALE_FACTOR_TO_RAY = 10 ** (RAY_DECIMALS - stDecimals);
        JT_SCALE_FACTOR_TO_RAY = 10 ** (RAY_DECIMALS - jtDecimals);
    }

    /// @inheritdoc RoycoKernel
    /// @dev Scale the ST asset quantity up to NAV units (RAY precision)
    function stConvertTrancheUnitsToNAVUnits(TRANCHE_UNIT _stAssets) public view override(RoycoKernel) returns (NAV_UNIT nav) {
        return toNAVUnits(toUint256(_stAssets) * ST_SCALE_FACTOR_TO_RAY);
    }

    /// @inheritdoc RoycoKernel
    /// @dev Scale the JT asset quantity up to NAV units (RAY precision)
    function jtConvertTrancheUnitsToNAVUnits(TRANCHE_UNIT _jtAssets) public view override(RoycoKernel) returns (NAV_UNIT nav) {
        return toNAVUnits(toUint256(_jtAssets) * JT_SCALE_FACTOR_TO_RAY);
    }

    /// @inheritdoc RoycoKernel
    /// @dev Scale the NAV quantity (RAY precision) down to ST asset units, rounding down
    function stConvertNAVUnitsToTrancheUnits(NAV_UNIT _nav) public view override(RoycoKernel) returns (TRANCHE_UNIT stAssets) {
        return toTrancheUnits(toUint256(_nav) / ST_SCALE_FACTOR_TO_RAY);
    }

    /// @inheritdoc RoycoKernel
    /// @dev Scale the NAV quantity (RAY precision) down to JT asset units, rounding down
    function jtConvertNAVUnitsToTrancheUnits(NAV_UNIT _nav) public view override(RoycoKernel) returns (TRANCHE_UNIT jtAssets) {
        return toTrancheUnits(toUint256(_nav) / JT_SCALE_FACTOR_TO_RAY);
    }
